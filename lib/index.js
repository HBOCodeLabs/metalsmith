...
/**
 * Add a file or files to the list of ignoreWrites.
 * ignoreWrites will be processed but not read or written as files in the build step.
 *
 * @param {String or Strings} The names of files or directories to ignore read/write.
 * @return {Metalsmith}
 */

 Metalsmith.prototype.ignoreWrite = function(files){
  if (!arguments.length) return this.ignoreWrites.slice();
  this.ignoreWrites = this.ignoreWrites.concat(files);
  return this;
 }
...
Metalsmith.prototype.readFile = unyield(function*(file){
  ...
  // Don't read this file if it's in the ignoreWrites array
  var ignoreWrites = this.ignoreWrites;
  for(var i = 0; i < ignoreWrites.length; i++) {
    if(minimatch(file, ignoreWrites[i])) {
      ret.contents = "";
      return ret;
    }
  }
 ...
 *
 * Data with empty contents (`''`) will not be written.
 *
 * @param {String} file
 * @param {Object} data
 */
